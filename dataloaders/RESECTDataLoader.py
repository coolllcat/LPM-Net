...
def sort_resect_dirs(names: List[str]) -> List[str]: 
    indices = sorted(
        range(len(names)), 
        key=lambda index: int(names[index][4:]) 
    ) 
...
class RESECTDataset(Dataset): 
    def __init__(self, root_dir: str, is_train: bool, mode1: str = 'T1', mode2: str = 'FLAIR', transforms=None) -> None:
        super().__init__() 

        _supported_domains = ['T1', 'FLAIR']
        ...
        sample_names = sort_resect_dirs(sample_names) 
        ...
        print("RESECT data split: train: {}, valid and test: {}".format(
    ...
    def _load_image_and_label(sample_dir: str, mode1: str, mode2: str): 
        ...
        return mode1_image.transpose(2,0,1), mode2_image.transpose(2,0,1)
    ...
    def __str__(self) -> str:
        return "RESECTDataset" 


if __name__ == '__main__':
    dataset = RESECTDataset('/data/postgraduate/wmw/FusionDataset/RESECT', True)
