...
def sort_prostateMRI_dirs(names: List[str]) -> List[str]: 
    indices = sorted(
        range(len(names)), 
        key=lambda index: int(names[index][3:6]) 
    ) 
...
class ProstateDataset(Dataset): 
    def __init__(self, root_dir: str, is_train: bool, mode1: str = 'mode1', mode2: str = 'mode2', transforms=None) -> None:
        super().__init__() 
        ...
        sample_names = sort_prostateMRI_dirs(sample_names) 
        ...
        print("ProstateMRI data split: train: {}, valid and test: {}".format(
...
if __name__ == '__main__':
    dataset = ProstateDataset('/data/postgraduate/wmw/FusionDataset/Prostate-Fused-MRI', True)
