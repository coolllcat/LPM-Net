...
class PMLNet(nn.Module):
    def __init__(self, args):
        ...
        self.cfc_mask = CfcCell(self.base_c*(self.depth_c+1), 2) if self.iscfc else nn.Conv3d(self.base_c*(self.depth_c+1), 2, kernel_size=1)
    ...
    def forward(self, x1, x2):
        ...
        # ********rec branch*************
        mask_size = 8
        D_mask, H_mask, W_mask = D//mask_size, H//mask_size, W//mask_size
        mask1 = (torch.rand(D_mask,H_mask,W_mask) > self.rec).float().unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).repeat(1,1,1,mask_size,mask_size,mask_size).permute(0,3,1,4,2,5).reshape(1,1,D,H,W).cuda()
        mask2 = (torch.rand(D_mask,H_mask,W_mask) > self.rec).float().unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).repeat(1,1,1,mask_size,mask_size,mask_size).permute(0,3,1,4,2,5).reshape(1,1,D,H,W).cuda()
        x1_mask = x1 * mask1
        x2_mask = x2 * mask2

        x_mask = torch.cat([x1_mask, x2_mask], dim=1)
        x_mask = self.Conv_in(x_mask)
        B, C, D, H, W = x_mask.shape

        out_P1_mask = self.Conv_P1(x_mask.permute(0,2,1,3,4).reshape(B*D, C, H, W))
        out_P2_mask = self.Conv_P2(x_mask.permute(0,3,1,2,4).reshape(B*H, C, D, W))
        out_P3_mask = self.Conv_P3(x_mask.permute(0,4,1,2,3).reshape(B*W, C, D, H))

        out_P1_mask = self.Mamba_P1(out_P1_mask)
        out_P2_mask = self.Mamba_P2(out_P2_mask)
        out_P3_mask = self.Mamba_P3(out_P3_mask)

        C = out_P1_mask.shape[1]
        out_P1_mask = out_P1_mask.reshape(B, D, C, H, W).permute(0,2,1,3,4)
        out_P2_mask = out_P2_mask.reshape(B, H, C, D, W).permute(0,2,3,1,4)
        out_P3_mask = out_P3_mask.reshape(B, W, C, D, H).permute(0,2,3,4,1)

        if self.fuse_method == 'mean':
            out_mask = (out_P1_mask + out_P2_mask + out_P3_mask) / 3
        elif self.fuse_method == 'min':
            out_mask = torch.min(torch.min(out_P1_mask, out_P2_mask), out_P3_mask)
        elif self.fuse_method == 'max':
            out_mask = torch.max(torch.max(out_P1_mask, out_P2_mask), out_P3_mask)

        if self.iscfc:
            out_mask, _, _ = self.cfc_mask(out_mask)
        else:
            out_mask = self.cfc_mask(out_mask)
        out_mask = F.tanh(out_mask) / 2 + 0.5

        # ********rec branch*************

        return out, base, detail, out_mask
